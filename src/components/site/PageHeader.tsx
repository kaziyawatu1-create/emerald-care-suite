...
export function PageHeader({ eyebrow, title, subtitle, children }: { eyebrow?: string; title: string; subtitle?: string; children?: ReactNode }) {
  return (
    <section className="relative overflow-hidden border-b border-border bg-gradient-to-br from-primary/5 via-background to-gold/5">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-12 md:px-8 md:py-16 text-center">
        {eyebrow && (
        ...
        <h1 className="mt-3 font-display text-4xl md:text-6xl font-bold leading-[1.05]">{title}</h1>
        {subtitle && <p className="mx-auto mt-3 max-w-2xl text-base md:text-lg text-muted-foreground">{subtitle}</p>}
